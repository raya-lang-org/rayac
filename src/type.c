...
static uint32_t hash_type(SType *t) {
    ...
    switch (t->kind) {
        ...
        case ST_SLICE:
            ...
            break;
        case ST_ARRAY:
            h = (h ^ (uint32_t)t->as.array.size) * 16777619;
...
static bool type_shape_eq(SType *a, SType *b) {
    ...
    switch (a->kind) {
        case ST_VOID: case ST_BOOL: case ST_NORETURN: case ST_OPAQUE:
            return true;
        case ST_INT:
            return a->as.integer.is_signed == b->as.integer.is_signed &&
                   a->as.integer.bits == b->as.integer.bits;
        case ST_FLOAT:
            return a->as.floating.bits == b->as.floating.bits;
        case ST_POINTER:
        case ST_REFERENCE:
        case ST_SLICE:
            return a->as.pointer.is_const == b->as.pointer.is_const &&
                   a->as.pointer.base == b->as.pointer.base;
        case ST_ARRAY:
            return a->as.array.size == b->as.array.size &&
                   a->as.array.base == b->as.array.base;
        case ST_OPTIONAL:
        case ST_ERROR_UNION:
            return a->as.optional.base == b->as.optional.base;
        case ST_FUNCTION:
        ...
        case ST_ENUM:
        case ST_TRAIT:
            if (!sv_eq(a->as.struct_.name, b->as.struct_.name)) return false;
            if (a->as.struct_.generic_arg_count != b->as.struct_.generic_arg_count) return false;
            return true;
        case ST_GENERIC_PARAM:
            return sv_eq(a->as.generic_param.name, b->as.generic_param.name) &&
                   a->as.generic_param.index == b->as.generic_param.index;
    }
    return false;
}

static SType *intern_type(TypeTable *tt, SType *proto) {
    uint32_t h = hash_type(proto) % TYPE_TABLE_BUCKETS;
    proto->hash = h;
    for (SType *t = tt->buckets[h]; t; t = t->next_hash) {
        if (t->hash == proto->hash && type_shape_eq(t, proto)) {
            return t;
        }
    }
...
SType *st_void(TypeTable *tt) {
    SType t = {.kind = ST_VOID};
    SType *p = arena_alloc(tt->arena, sizeof(SType));
    memcpy(p, &t, sizeof(t));
    return intern_type(tt, p);
}

SType *st_bool(TypeTable *tt) {
    SType t = {.kind = ST_BOOL};
    SType *p = arena_alloc(tt->arena, sizeof(SType));
    memcpy(p, &t, sizeof(t));
    return intern_type(tt, p);
}
...
bool st_can_coerce(SType *from, SType *to) {
    ...
    if (from->kind == ST_REFERENCE && to->kind == ST_REFERENCE) {
        if (!from->as.reference.is_const && to->as.reference.is_const)
            return st_eq(from->as.reference.base, to->as.reference.base);
    }
    return false;
}

static bool sv_eq_cstr(StringView sv, const char *cstr) {
...
const char *st_name(SType *t) {
    ...
    switch (t->kind) {
        ...
        case ST_INT: {
            snprintf(buf, 256, "%c%u", t->as.integer.is_signed ? 'i' : 'u', t->as.integer.bits);
            return buf;
        }
        case ST_FLOAT: {
            snprintf(buf, 256, "f%u", t->as.floating.bits);
            return buf;
        }
        case ST_NORETURN: return "noreturn";
        case ST_POINTER: {
            const char *base = st_name(t->as.pointer.base);
            snprintf(buf, 256, "*%s%s", t->as.pointer.is_const ? "const " : "", base);
            return buf;
        }
        case ST_REFERENCE: {
            const char *base = st_name(t->as.reference.base);
            snprintf(buf, 256, "&%s%s", t->as.reference.is_const ? "const " : "", base);
            return buf;
        }
        case ST_SLICE: {
            const char *base = st_name(t->as.slice.base);
            snprintf(buf, 256, "[]%s%s", t->as.slice.is_const ? "const " : "", base);
            return buf;
        }
        case ST_ARRAY: {
            const char *base = st_name(t->as.array.base);
            snprintf(buf, 256, "[%llu]%s", (unsigned long long)t->as.array.size, base);
            return buf;
        }
        case ST_OPTIONAL: {
            const char *base = st_name(t->as.optional.base);
            snprintf(buf, 256, "?%s", base);
            return buf;
        }
        case ST_ERROR_UNION: {
            const char *base = st_name(t->as.error_union.base);
            snprintf(buf, 256, "!%s", base);
            return buf;
        }
        case ST_FUNCTION: {
            const char *ret = st_name(t->as.function.ret);
            snprintf(buf, 256, "fn(...) -> %s", ret);
            return buf;
        }
        case ST_STRUCT:
        case ST_UNION:
        case ST_ENUM:
        case ST_TRAIT:
            if (t->as.struct_.name.data) {
                snprintf(buf, 256, "%.*s", (int)t->as.struct_.name.len, t->as.struct_.name.data);
                return buf;
            }
            return "<anon>";
        case ST_GENERIC_PARAM:
            if (t->as.generic_param.name.data) {
                snprintf(buf, 256, "%.*s", (int)t->as.generic_param.name.len, t->as.generic_param.name.data);
                return buf;
            }
            return "<T>";
        case ST_OPAQUE:
...
SType *st_from_ast(TypeTable *tt, AstNode *type_expr) {
    if (!type_expr) return st_void(tt);
    switch (type_expr->kind) {
        case AST_TYPE_NAMED: {
            StringView name = type_expr->as.type_named.name;
            if (sv_eq_cstr(name, "void"))   return st_void(tt);
            if (sv_eq_cstr(name, "bool"))   return st_bool(tt);
            if (sv_eq_cstr(name, "isize"))  return st_int(tt, true, 64);
            if (sv_eq_cstr(name, "usize"))  return st_int(tt, false, 64);
            if (name.len > 1 && name.data[0] == 'i') {
                int bits = atoi(name.data + 1);
                if (bits == 8 || bits == 16 || bits == 32 || bits == 64 || bits == 128)
                    return st_int(tt, true, bits);
            }
            if (name.len > 1 && name.data[0] == 'u') {
                int bits = atoi(name.data + 1);
                if (bits == 8 || bits == 16 || bits == 32 || bits == 64 || bits == 128)
                    return st_int(tt, false, bits);
            }
            if (name.len > 1 && name.data[0] == 'f') {
                int bits = atoi(name.data + 1);
                if (bits == 32 || bits == 64)
                    return st_float(tt, bits);
            }
            if (sv_eq_cstr(name, "noreturn")) {
                SType t = {.kind = ST_NORETURN};
                SType *p = arena_alloc(tt->arena, sizeof(SType));
                memcpy(p, &t, sizeof(t));
                return intern_type(tt, p);
            }
            SType t = {.kind = ST_STRUCT};
            t.as.struct_.name = name;
            SType *p = arena_alloc(tt->arena, sizeof(SType));
            memcpy(p, &t, sizeof(t));
            return intern_type(tt, p);
        }
        case AST_TYPE_POINTER:
            return st_pointer(tt, type_expr->as.type_pointer.is_const,
                st_from_ast(tt, type_expr->as.type_pointer.base));
        case AST_TYPE_REFERENCE:
            return st_reference(tt, type_expr->as.type_reference.is_const,
                st_from_ast(tt, type_expr->as.type_reference.base));
        case AST_TYPE_SLICE:
            return st_slice(tt, type_expr->as.type_slice.is_const,
                st_from_ast(tt, type_expr->as.type_slice.base));
        case AST_TYPE_ARRAY:
            return st_array(tt, type_expr->as.type_array.size,
                st_from_ast(tt, type_expr->as.type_array.base));
        case AST_TYPE_OPTIONAL:
            return st_optional(tt, st_from_ast(tt, type_expr->as.type_optional.base));
        case AST_TYPE_ERROR_UNION:
            return st_error_union(tt, st_from_ast(tt, type_expr->as.type_error_union.base));
        case AST_TYPE_FUNCTION: {
            size_t pc = type_expr->as.type_function.params.count;
            SType **params = arena_alloc(tt->arena, pc * sizeof(SType*));
            for (size_t i = 0; i < pc; i++) {
                params[i] = st_from_ast(tt, type_expr->as.type_function.params.nodes[i]);
            }
            SType *ret = st_from_ast(tt, type_expr->as.type_function.ret);
            return st_function(tt, params, pc, ret, false);
        }
        default:
            return st_void(tt);
    }
